...
public class AnSlot extends AnCommand {
	...
	public Boolean process() {
		...
		if (!plugin.permission.isAdmin(player)
				&& !plugin.permission.canCreate(player)) {
			noPermission();
			return true;
		}
			...
			if (plugin.typeData.isType(args[3])) {
				...
				if (plugin.permission.canCreate(player, type)) {
					...
					if (plugin.economy.has(name, cost)) {
						...
						sendMessage("You don't have enough money. Cost: "
								+ cost);
					}
		...
		// Set slot controller
			...
			if (!plugin.economy.has(player.getName(), amount)) {
				...
				// Enable if needed
				if ((slot.getFunds() + amount) > plugin.typeData
						.getMaxPrize(slot.getType())) {
			...
			if (slot.getFunds() < amount) {
				sendMessage("Not enough funds in " + slot.getName()
						+ ". Withdrawing all available funds.");
				amount = slot.getFunds();
			}

			// Disable if necessary
			if ((slot.getFunds() - amount) < plugin.typeData.getMaxPrize(slot
					.getType())) {
	...
	private void usage() {
		...
		String[] messages = { "Usage:", "/casino slot <slot> type <type>",
