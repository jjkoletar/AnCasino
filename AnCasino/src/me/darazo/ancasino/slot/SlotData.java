...
public class SlotData {
	...
	public void loadSlots() {
		...
		if (plugin.configData.slots.isConfigurationSection("slots")) {
			Set<String> slots = plugin.configData.slots
					.getConfigurationSection("slots").getKeys(false);
			if (!slots.isEmpty()) {
	...
	private void loadSlot(String name) {
		...
		SlotMachine slot = new SlotMachine(name, type, owner, world, managed,
				blocks, controller, funds);
		addSlot(slot);
	}
	...
	private ArrayList<Block> getBlocks(String name) {
		...
		List<String> xyz = plugin.configData.slots.getStringList("slots."
				+ name + ".location");
		ArrayList<Block> blocks = new ArrayList<Block>();
		World world = Bukkit.getWorld(plugin.configData.slots.getString(
				"slots." + name + ".world", "world"));

		for (String coord : xyz) {
			String[] b = coord.split("\\,");
			Location loc = new Location(world, Integer.parseInt(b[0]),
	...
	private Block getController(String name) {
		...
		String location = plugin.configData.slots.getString("slots." + name
				+ ".controller");
		World world = Bukkit.getWorld(plugin.configData.slots
